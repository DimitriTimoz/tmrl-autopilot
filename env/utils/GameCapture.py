...
from mss import mss
from tmrl_autopilot.env.utils.constants import GAME_WINDOW_NAME
...
class GameViewer:
    ...
    def process_screen(self, screenshot: np.ndarray) -> np.ndarray:
        baw = cv2.cvtColor(screenshot, cv2.COLOR_RGB2BGR)[0]

        baw = cv2.resize(baw, (256, 256))
        return baw
    ...
    def get_obs(self):
        ...
        return np.array(processed_img).astype(np.float32)
