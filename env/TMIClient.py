...
class SimStateClient(Client):
    ...
    Its only job is to get the simulation state that is used by the gym tmrl_autopilot.env for reward computation.
    """
...
class ThreadedClient:
    """
    Allows to run the Client in a separate thread, so that the gym tmrl_autopilot.env can run in the main thread.
    """
